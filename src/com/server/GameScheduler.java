...
public class GameScheduler extends TimerTask {
	 ...
	 * execute every 500 ticks. Executes commands from every client, and the ai
	 * schedule. Checks for game end conditions once every 1000 ticks.
	 * 
	 * @param time
	 *            the time step.
	 */
	...
	/**
	 * Checks to see if any of the Game End condions are met at the input time.
	 * The game is over if the time limit is up, if there are no players and the
	 * time is past one minute, if all the players in the game have finished
	 * their quests, if all players are dead and there are more than one player,
	 * or if there are no living monsters.
	 * 
	 * @param time
	 *            the time to check if the game is over at.
