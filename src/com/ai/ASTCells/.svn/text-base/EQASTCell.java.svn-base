 ...
 * 
 */
public class EQASTCell extends AbstractASTCell {
	 ...
	 */
	public EQASTCell(ASTCell lhs, ASTCell rhs){
	...
	public ASTCell execute(VM c) throws ExecutionException {
		if(DEBUG){System.out.println("EQASTCell.execute : "+_lhs.print()+" "+_rhs.print());}
		
		
		_val = _lhs.execute(c).equals(_rhs.execute(c));
		if(DEBUG){System.out.println("EQASTCell.execute produce _val = "+ _val);}
