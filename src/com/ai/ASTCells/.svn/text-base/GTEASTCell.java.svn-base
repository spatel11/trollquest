 ...
 * 
 */
public class GTEASTCell extends AbstractASTCell {
	 ...
	 */
	public GTEASTCell(ASTCell lhs, ASTCell rhs){
		_lhs = lhs;
		_rhs = rhs;
	}
	/**
	...
	public ASTCell execute(VM c) throws ExecutionException {
		try {
			if(DEBUG){System.out.println("GTASTCELL.execute() lhs.numVal: "+ _lhs.execute(c).numVal()+" >= "+_rhs.execute(c).numVal());}
			_val = _lhs.execute(c).numVal() >= _rhs.execute(c).numVal();
		} catch (TypeException e) {
			throw new ExecutionException("GTASTCELL.execute() failed to get numberValues for both sides");
		}
